...
public class ImageRadarView extends View {
    ...
    public ImageRadarView(Context context, AttributeSet att) {
        ...
        backDrawable = backDrawable == null ? getResources().getDrawable(
                R.mipmap.radar_back) : backDrawable;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawCircle(
                cx,
                cy,
                radius * ((float) (degree / circleExpandReverseSpeed % circleExpandReverseSpeed)
                        / (float) circleExpandReverseSpeed),
                sweepPaint);
        canvas.drawBitmap(
                radar_top_src,
                cx - radar_top_src.getWidth() / 2,
                cx - radar_top_src.getHeight() / 2,
                null);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //根据图片大小设置控件大小
        int d = radar_back_src.getWidth() >
                radar_back_src.getHeight() ?
                radar_back_src.getWidth() :
                radar_back_src.getHeight();
